 ...
 * @name: 文件
 * @Author: Gin
 * @Date: 2021-07-02 11:28:46
 * @LastEditors: Gin
 * @LastEditTime: 2021-07-02 14:36:19
 */
export default {
  inserted(el, vDir, vNode) {
    ...
    el.addEventListener('keypress', event => {
      ...
      console.log('!re.test(inputKey) && !e.ctrlKey: ', !re.test(inputKey) && !e.ctrlKey)
      console.log(content.indexOf('.') > 0 && inputKey === '.')
    ...
    el.addEventListener('focusout', event => { // 此处会在 el-input 的 @change 后执行
      ...
      e.target.value = parseFloat(content.toFixed(arg_precision))
      setVal(parseFloat(content.toFixed(arg_precision)))
      // -- callback写法1
